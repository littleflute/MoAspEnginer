...
<form name="form1" method="POST">
  <table width="100%" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#000000">
    <tr bgcolor="#B1E1F3"> 
      <td height="30" colspan="4"> 
        <div align="center"><strong>所有民政新闻</strong></div></td>
  </tr>
  <tr> 
    <td width="53%"><div align="center">新闻文件名</div></td>
  ...
  <%   dim currentpage,id,page_total,i 
   sql="select id, nstitle,times from news order by times desc"
    ...
    %>
  <tr> 
    <td width="53%"><div align="center"><a href="../listnews.asp?id=<%=rs("id")%>" target="_blank"><%=rs("nstitle")%></a></div></td>
    <td width="27%"><div align="center"><%=rs("times")%></div></td>
    <td width="10%"><div align="center"><a href="editmznew.asp?id=<%=rs("id")%>">修改</a>/<a href="delmznews.asp?id=<%=rs("id")%>">删除</a></div></td>
  ...
  %>
  <tr>
    <td colspan="4"> <div align="center"> &nbsp;&nbsp; <a href="addmznew.asp" target="_blank">添加内容</a>&nbsp;&nbsp; 
          <input type="button" value="全 选" onclick="sltAll()" name=button1>
        &nbsp;&nbsp;
        <input type="button" value="清 空" onclick="sltNull()" name=button2>
        &nbsp;&nbsp;
        <input type="submit" value="删 除" name="tijiao" onclick="SelectChk('delmznews.asp')">
      </div></td></tr>
  <tr> 
    <td colspan="4">
	<%if page_total>1 then
	       ...
	       response.write"上一页"
	       else %> <a href="admin_news.asp?page=<%=currentPage-1%>">上一页</a> 
                        <%
	                   end if  %>
		   <script>
...
<select  onChange=GoToWhere(this) name=select5 class="new">
		  ...
		  %>
            <option value="admin_news.asp?page=<%=i%>" selected><%=i%></option>
            <% else %>
			<option value="admin_news.asp?page=<%=i%>"><%=i%></option>
			<%  end if 
			next %>
          </select>
		  <% if currentPage=page_total then
		response.write"下一页"
		else %> <a href="admin_news.asp?page=<%=currentPage+1%>">下一页</a> 
                        <% end if
