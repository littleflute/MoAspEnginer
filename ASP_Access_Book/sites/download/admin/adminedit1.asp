...
<FONT color="#FF9933">点击相关软件察看及修改软件资料，删除软件请选中相关软件然后点击删除
</FONT>
<form name="searchsoft" method="POST" action="adminedit1.asp">
查找软件:  <input class=smallInput type="text" name="txtitle" size="13"><input class=buttonface type="submit" value="查 询" name="title">
</form>
<form method=Post action="adminedit1.asp">
 ...
 <td width="400" align="center"><STRONG><FONT color="#FFFFFF">软 件 名</FONT></STRONG></td>
      ...
      <%
	if title<>"" then
	sql="select * from news where newsnote like '%"&trim(newsnote)&"%' order by id desc"
	else
	...
	rs.open sql,conn,1,1
  	...
  	if rs.eof and rs.bof then
      		...
      		if (currentpage-1)*MaxPerPage>totalput then
       		...
       		if currentPage=1 then
           		showpage totalput,MaxPerPage,"adminedit1.asp"
            		showContent
            		showpage totalput,MaxPerPage,"adminedit1.asp"
       		else
          		if (currentPage-1)*MaxPerPage<totalPut then
...
%>
      ...
      <p><a href="editsoft1.asp?id=<%=rs("id")%>"><%=rs("newsnote")%></a>
      </td>
