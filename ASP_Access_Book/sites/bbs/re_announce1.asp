...
<center>
  ...
  <table border="1" align="center" cellpadding="0" cellspacing="0" bgcolor="#eeeeee" bordercolorlight="#C0C0C0" bordercolordark="#FFFFFF" width=102%>
    <form method="post" action="" name="myform" >
      <tr> 
        <td width="11%" class="p9">主题：</td>
        <td width="89%">
          <input name="title" type="text" class="p9" value="re:<%=title%>" size="80">
          ** </td>
      </tr>
      <tr> 
      ...
      <tr> 
        <td class="p9">内容：</td>
        <td>
          <textarea name="body" rows="15" cols="70" wrap="soft"></textarea>
      ...
      <tr> 
        <td></td>
        <td>
          <input type="submit" value="　提交　" size="20" style="font-family: 宋体; font-size: 9pt; background-color: #FFFFFF; border-style: solid; border-width: 1">
        </td>
      </tr>
    </form>
  </table>
	</center>
	<p align=center><a href="particular.asp?bbs_id=<%=bbs_id%>">返回上一页</a>
	<%
	if request("title")<>"" and request("user_name")<>"" then
		dim body,layer,parent_id,child,hits,ip,user_name,face           '定义变量方便使用
		title=request.form("title")                                '返回文章标题
		body=ubbcode(request.form("body"))                                  '返回文章内容
		user_name=request.form("user_name")                        '返回作者姓名
		layer=2 
		face=request.form("face")                                                   '这是第二层
		...
		'以下将文章保存到数据库
		dim sql,svalues
		sql = "Insert into news(title,layer,parent_id,child,hits,ip,user_name,submit_date"
		...
		db.execute(sql)
		'下面两句将原文章的回复数加1
		sql="update news set child=child+1 where bbs_id=" & bbs_id
		db.execute(sql)
		db.close
		'重定向回原来页面
